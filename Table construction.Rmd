...
features0=df_beforeCOMPAS%>%
         distinct(person_id,first,last,age,sex,marital_status,race,
...
Overlap functions
```{r}
#boolean fcn for if date 1 and date 2 overlap
#( start1 <= end2 and start2 <= end1 ), 
#date ranges (start1,end1) and (start2,end2)
#(row1$OffenseDate, row1$JudgementDate) and (row2$OffenseDate, row2$JudgementDate)
simple_overlap<-function(row1,row2){
  if (row1$OffenseDate<=row2JudgementDate & row2$OffenseDate<= row1$JudgementDate){
    return (TRUE)
  }
  return (FALSE)
}

#compares one date interval to a whole df of date intervals, returns true if date interval overlaps with ANY of other date intervals
df_overlap<-function(df,row){
  bool_vec=sapply(df,simple_overlap(.,row))
  if(TRUE %in% bool_vec){return(TRUE)}
  return(FALSE)
}
...
.	Pending charge at time of offense (pending: at time of offense, had another charge for which no disposition was yet reached)
     ...
     *A lawsuit is said to be pending from its inception until the issuance of a final judgment by a court. The phrase pending appeal refers to the time before an appeal is taken, as well as to the period during which an appeal is in progress.*
     ...
     MUST FFIX 2 SCREENING DATES ISSUE
```{r}
...
offense_interval=df_beforeCOMPAS%>%
                 distinct(person_id,case_number,charge_number,charge_degree,offense_date)%>%
  
                 left_join(convictions_subset, by=c("person_id","case_number","charge_number"))%>%
                  
                 mutate(interval=interval(start=offense_date,end=JudgementDate))%>%
                 group_by(person_id)%>%
                    summarise(offense_intervals=list(interval))
...
features<-features0%>%
           ...
           mutate(p_vio_noage=p_violence_raw-age_poly_violence(p_current_age))
          #  mutate(#failure to appear weighted features 
          ...
          #         arnol_nvca=case_when(arnold_nvca_raw<=3~"No",
          ...
          #                        prior_convF_nca,prior_convVIO_nca,fta_pretrial2yr_nca,
          #                        prior_incarceration_nca,
          #                  arnold_nvca_raw,current_age_nca,pending_charge_nca,
          #                        prior_convM_nca,prior_convF_nca,prior_convVIO_nca,
          #                        fta_pretrial2yr_nca,prior_incarceration_nca)
          #        )
