...
testarnold<-df_beforeCOMPAS%>%
           distinct(person_id,screening_date,
                    ,dob,statute,prison_in_custody,prison_out_custody)%>%
           mutate(current_age=screening_date-dob)%>%
                  # is_violent=switch(substr(statute,1,3),"777"=1,"782"=1,"784"=1,"806"=1,"812"=1,"825"=1,"827"=1,NA=NA, x=0))
                  # days_from_compas=screening_date-offense_date,
                  # is_current=if_else(days_from_compas<5,1,0),
                  in_prison=switch(prison_out_custody-prison_in_custody,1,0),
                  # twenty_or_less=if_else(current_age<=20,1,0)
                  # 
                  # )%>%
           as_data_frame()
