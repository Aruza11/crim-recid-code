...
df_screendate=dfnew%>%
        ...
        mutate(gap=as.numeric(screening_date-COMPAS_offense_date,units="days"),
               COMPAS_offense_date=if_else(gap>30,NA,COMPAS_offense_date),
               beforeCOMPAS=if_else(offense_date<COMPAS_offense_date,1,0)
              )

df_beforeCOMPAS=df_screendate%>%filter(beforeCOMPAS==1)
        
max(select(df_beforeCOMPAS,gap))
boxplot(select(df_beforeCOMPAS,gap))
fivenum(df_beforeCOMPAS$gap)
kurt=filter(df_beforeCOMPAS,person_id==7294) #somebody with 2 screening dates

weird_gap=filter(df_beforeCOMPAS, gap<=38&gap>30)
large_gap_ids=unique(weird_gap$person_id)

brandon=filter(df_beforeCOMPAS, person_id==193)
#problem: people can have multiple screening dates--how to deal with this? 
...
features<-features%>%
          ...
          rename(violence_score=type_of_assessment,
                 violence_raw=raw_score,violence_decile=decile_score)%>%
  
          left_join(propub_recidivism_scores,by="person_id")%>%
          rename(recid_score=type_of_assessment,
                 recid_raw=raw_score,recid_decile=decile_score)%>%

          left_join(propub_failure_scores,by="person_id")%>%
          rename(fail_to_appear_score=type_of_assessment,
                 fail_to_appear_raw=raw_score,fail_to_appear_decile=decile_score)
...
#rows in x with no entries in y have NA values in the new columns
