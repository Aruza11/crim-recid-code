...
NOTES: 1) DFNEW ONLY CONTAINS INFORMATION FROM BEFORE THE COMPAS EVALUATION (so we can perform predictive analysis on the code). 
...
**Note to self: (11714-11109)/11109 people were screened 2x
...
**Note to self: must call as_data_frame() before group_by function, or else I get a parser stack overflow error. Perhaps because it's trying to invoke the function on each element in the group_by? 
**Note to self: do not use filter(is_felony==1) to remove entries because I want 0s for when the felony count is  0, not NAs (when I left join it later)
...
dynamic_fel=df_beforeCOMPAS%>%
           ...
           # filter(is_felony==1)%>%
           mutate(less_5yr = if_else(screening_date-date_charge_filed<=5,1,0))%>%
  
           distinct(person_id,charge_degree,case_number,charge,is_felony,less_5yr) %>%
  #evreything can be viewed up to this point, after this I get a "parser stack overflow" error
           as_data_frame()%>%
           group_by(person_id) %>%
             summarize(felony_count_person=sum(is_felony),
                       yrs5_felcount_person = sum(less_5yr))
...
dynamic_incarceration=df_beforeCOMPAS %>%
                        ...
                        mutate(prison_time=prison_out_custody-prison_in_custody,
                               jail_time=jail_out_custody-jail_in_custody)%>%
                        as_data_frame()%>%

                        group_by(person_id) %>%
                           summarize(prison_visits=n_distinct(prison_in_custody),
                                     prison_time=n_distinct(prison_time),
  
                                     jail_visits=n_distinct(jail_in_custody),
                                     jail_time=n_distinct(jail_time),
                                     numcases=n_distinct(case_number)
                                     )



# testperson=filter(dfnew,person_id==75)
# numfel=distinct(testperson,person_id,offense_date,charge_degree,prison_in_custody,prison_out_custody,jail_in_custody,jail_out_custody,case_number,charge)%>%
#               mutate(is_felony = if_else(substr(charge_degree,2,2)=="F",1,0)) %>%
#               distinct(is_felony,case_number,charge_degree,charge)
                 # summarize(felony_count_person = distinct(is_felony,case_number,charge_degree),is_felony))
...
dynamic_drug=df_beforeCOMPAS%>%
                 ...
                 distinct(person_id,charge_dis_drug_5yrsegree,case_number,charge,is_drug,is_drug_5yrs) %>%
                 as_data_frame()%>%
...
*Age is in years, 
Code status 2/4/18: code seems to work, but I suspect that I may have handled something wrong. 
1) deal with NAs for prison incarceration (should appear as NA, NOT 0s)
2) Date subtraction not working as expected
```{r}
#CHECK IF SOMEBODY DOESN'T HAVE ANY OFFENSES WITHIN 30 DAYS
...
# 
           mutate(current_violent_offense=if_else(is_violent==1,1,0),
...
testarnold<-df_beforeCOMPAS%>%
           distinct(person_id,screening_date,offense_date,dob,statute,prison_in_custody,prison_out_custody)%>%
  as_data_frame()%>%


# testarnold$screening_date<-as.Date(testarnold$screening_date,format = "%Y-%m-%d %T")
# testarnold$offense_date<-as.Date(testarnold$offense_date,format = "%Y-%m-%d %T")
# testarnold$dob<-as.Date(testarnold$dob,format = "%Y-%m-%d %T")
# testarnold$prison_in_custody<-as.Date(testarnold$prison_in_custody,format = "%Y-%m-%d %T")
# testarnold$prison_out_custody<-as.Date(testarnold$prison_out_custody,format = "%Y-%m-%d %T")

# testarnold<-testarnold%>%
           mutate(screening_date=ymd_hms(screening_date),
                  ...
                  current_age=as.period(screening_date-dob,units="years"),
                  # is_violent=if_else(substr(statute,1,3) %in% c("777","782","784","794","806","812","825","827"),1,0),
                  # days_from_compas=as.numeric(screening_date-offense_date,units="days"),
                  # is_current=if_else(days_from_compas<30,1,0),
                  # prison_time=prison_out_custody-prison_in_custody,
                  # in_prison=case_when(is.null(prison_in_custody)~NA, prison_out_custody-prison_in_custody>0~1,prison_out_custody-prison_in_custody<0~0),
                  # twenty_or_less=if_else(current_age<=20,1,0)
                  )
           # as_data_frame()
...
Possible issue:  person_id.x!=person_id.y. Check why. Also, why are some person_ids missing?
Note: must convert to dataframe type right away because left_join only works on tbls from the same source. That's also why I have to coerce date columns from character to Date class. 
```{r}
...
recid$screening_date<-as.Date(recid$screening_date,format = "%Y-%m-%d %T")
recid$jail_in_custody<-as.Date(recid$jail_in_custody,format = "%Y-%m-%d %T")
recid$jail_out_custody<-as.Date(recid$jail_out_custody,format = "%Y-%m-%d %T")
recid$offense_date<-as.Date(recid$offense_date,format = "%Y-%m-%d %T")
#converted to date class explicitly, date subtraction counts years
recid<-recid%>%
            mutate(is_felony = if_else(substr(charge_degree,2,2)=="F",1,0),
                   is_misdem= if_else(substr(charge_degree,2,2)=="M",1,0),
                   is_violent=if_else(substr(statute,1,3) %in% c("777","782","784","794","806","812","825","827"),1,0),
                   jailtime_in_days=jail_out_custody-jail_in_custody,
                   recid_jail=if_else((jail_in_custody-screening_date>30)&!is.na(jail_in_custody)&!is.na(screening_date),1,0),
                   within_two_years=if_else(offense_date-screening_date<720,1,0),
                   is_traffic = if_else(charge_degree=="(TCX)",1,0),
                   is_municipal= if_else((!substr(statute,1,1)%in%c(0:9))&!is.na(statute),1,0),
                   
                   propub_recid=if_else((recid_jail==1&within_two_years==1&is_traffic==0&is_municipal==0),1,0),
