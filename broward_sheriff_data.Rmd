...
#original plot 
ggplot(features, aes(x=p_felony_count_person+p_misdem_count_person, y=p_violence_raw)) +
       geom_point(shape=1)  +
       ggtitle("Risk of Violence Raw vs Felonies per Person (original)")


for(w in c(.5,1,2,5,10,20)){

    no_age=features%>%
              select(person_id, p_age, p_felony_count_person,p_misdem_count_person,p_violence_raw)%>%
              mutate(raw_minus_age=p_violence_raw-w*exp_reg(p_age))
...
```{r}
exp_reg=exp(cf[2]*p_age+cf[1])-5

no_age_misdem=features%>%
          select(person_id, p_age, cq_misdem_count_person,p_violence_raw)%>%
          mutate(raw_minus_age=p_violence_raw-exp_reg)
      
  #original plot 
ggplot(features, aes(x=misdem_count_person, y=p_violence_raw)) +
       geom_point(shape=1)  +
...
```{r}
#felonies
ggplot(features, aes(x=felony_count_person, y=p_violence_raw)) +
     geom_point(shape=1)  +
     ggtitle("Risk of Violence Raw vs Number Felonies")


ggplot(features, aes(x=yrs5_felcount_person, y=p_violence_raw)) +
     geom_point(shape=1)  +
     ggtitle("Risk of Violence Raw vs Number Felonies")
...
ggplot(features, aes(x=misdem_count_5yrs, y=p_violence_raw)) +
     geom_point(shape=1)  +
     ggtitle("Risk of Violence Raw vs Number Misdemeanors")
