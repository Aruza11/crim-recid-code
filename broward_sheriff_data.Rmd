...
# ggplot(features, aes(x=p_age, y=p_violence_decile)) +
#      geom_point(shape=1)  +
#      ggtitle("Risk of Violence Score vs Age")
# 
# ggplot(features, aes(x=p_age, y=p_recid_decile)) +
#      geom_point(shape=1)  +
#      ggtitle("Risk of Recidivism Score vs Age")
# 
# ggplot(features, aes(x=p_age, y=p_fail_to_appear_decile)) +
#      geom_point(shape=1)  +
...
plot(age_subset$p_age, log(age_subset$p_violence_raw+5))
plot(log(age_subset$p_age), log(age_subset$p_violence_raw+5))
...
exponential.model <- lm(log(age_subset$p_violence_raw+5)~ age_subset$p_age)
    ...
    exp_reg_manual=function(x){exp(-0.032*x+1.55)-5}
    
    # plot(exp_reg,1,70) #the equation of the exponential regression 

exponential.model2 <- lm(log(age_subset$p_violence_raw+5)~ log(age_subset$p_age)) 
...
plot(age_subset$p_age, age_subset$p_violence_raw)
lines(age_subset$p_age, violraw.exponential-5,lwd=2, col = "blue", xlab = "Age", ylab = "Violence Raw Score")
lines(age_subset$p_age, violraw.exponential2-5,lwd=2, col = "green", xlab = "Age", ylab = "Violence Raw Score")
...
ggplot(features, aes(x=p_age, y=p_violence_raw)) +
     ...
     # stat_function(fun = exp_reg_manual, colour="red")+ 
     # stat_function(fun = exp_reg2, colour="green") +  
     xlim(18,70)
...
```{r}
# write.csv(select(age_subset, p_age, p_violence_raw),"age_violence_score.csv")
```
...
for(n in c(0,5,10,15,20,25,30,35)){
    ...
    print(ggplot(n_5yrfel, aes(x=p_age, y=p_violence_raw)) +
              geom_point(shape=1)  +
...
for(n in c(0:5)){
    ...
    print(ggplot(n_5yrfel, aes(x=p_age, y=score_no_age)) +
            geom_point(shape=1) +
...
corrected_5yrmisdem=features%>%
              select(person_id, p_age, p_yrs5_felcount_person,p_misdem_count_5yrs,p_violence_raw)%>%
              mutate(score_1=p_violence_raw-exp_reg(p_age))
...
corrected2_5yrmisdem=features%>%
              select(person_id, p_age, p_yrs5_felcount_person,p_misdem_count_5yrs,p_violence_raw)%>%
              mutate(score_2=p_violence_raw-exp_reg(p_age)-lin_reg(p_yrs5_felcount_person))
...
for(n in c(0:10)){
    ...
    print(ggplot(n_5yrmisdem, aes(x=p_age, y=p_violence_raw)) +
              geom_point(shape=1)  +
...
corrected_appear=features%>%
              ...
              mutate(total_fail_appear=cq_fail_appear_two_plus+cq_fail_appear_two_yr,
                     score_no_age=p_violence_raw-exp_reg(p_age),
                     score_no_age2=p_violence_raw-exp_reg(p_age)-lin_reg(p_yrs5_felcount_person),
                     score_no_age3=p_violence_raw-exp_reg(p_age)-
                                   lin_reg(p_yrs5_felcount_person)-
                                   lin_reg2(p_misdem_count_5yrs),
                     score_no_age4=p_violence_raw-exp_reg(p_age)-
                                   lin_reg2(p_misdem_count_5yrs)
                    )
...
corrected_drug=features%>%
              ...
              mutate(total_fail_appear=cq_fail_appear_two_plus+cq_fail_appear_two_yr,
                     score_no_age=p_violence_raw-exp_reg(p_age),
                     score_no_age2=p_violence_raw-exp_reg(p_age)-
                                   lin_reg(p_yrs5_felcount_person),
                     ...
                     score_no_age5=p_violence_raw-exp_reg(p_age)-
                                   lin_reg(p_yrs5_felcount_person)-
                                   lin_reg2(p_misdem_count_5yrs)-
                                   lin_reg3(total_fail_appear)
                    )
...
drug_linear.model <- lm(drug_subset$score_no_age~ drug_subset$p_drug_count_5yrs)
    ...
    lin_reg4=function(x){drug_cf[2]*x+drug_cf[1]} #x=failure to appear counts
...
Looking at failure to appear score
...
#Risk of failure to appear  and total number of failures to appear 
ggplot(features, aes(x=cq_fail_appear_two_yr+cq_fail_appear_two_plus, y=p_fail_to_appear_decile)) +
     geom_point(shape=1)  +
     ggtitle("Risk of Failure to Appear decile vs Total# Failure to appear")
