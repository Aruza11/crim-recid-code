...
cf=exponential.model%>%
   coef()%>%
   as.vector()
plot(function(x){cf[1]*x+cf[2]},1,70)


violraw.exponential2 <- exp(predict(exponential.model,list(Age=age_subset$age)))
plot(age_subset$age, age_subset$violence_raw+5)
lines(age_subset$age, violraw.exponential2,lwd=2, col = "red", xlab = "Age", ylab = "Violence Raw Score")
