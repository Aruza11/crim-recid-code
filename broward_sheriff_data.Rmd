...
```{r}
age_subset=features%>%
           select(person_id, first, last, age, violence_raw)%>%
           filter(age>18&age<=70)%>%
           group_by(age)%>%
              arrange(violence_raw,.by_group=TRUE)%>%
              top_n(n=-5,wt=violence_raw)
...
exponential.model <- lm(log(age_subset$violence_raw+5)~ age_subset$age)
...
plot(age_subset$age, age_subset$violence_raw)
lines(age_subset$age, violraw.exponential2-5,lwd=2, col = "red", xlab = "Age", ylab = "Violence Raw Score")
...
A function which takes in the features table, subtracts out the age variable from the *violence raw score* and plots the before and after plots. 
```{r}

exp_reg=function(x){exp(cf[2]*x+cf[1])-5} #x=age

no_age=features%>%
          select(person_id, age, felony_count_person,violence_raw)%>%
          mutate(raw_minus_age=violence_raw-exp_reg(age))
      
  #original plot 
ggplot(features, aes(x=felony_count_person, y=violence_raw)) +
       geom_point(shape=1)  +
       ggtitle("Risk of Violence Raw vs Felonies per Person (original)")
  
  #plot with no age  
ggplot(no_age, aes(x=felony_count_person, y=raw_minus_age)) +
      geom_point(shape=1)  +
      ggtitle("Risk of Violence Raw vs Felonies per Person (no age)")
  

```

Same as above but with misdemeanors
```{r}
exp_reg=exp(cf[2]*age+cf[1])-5

no_age_misdem=features%>%
          select(person_id, age, misdem_count_person,violence_raw)%>%
          mutate(raw_minus_age=violence_raw-exp_reg)
      
  #original plot 
ggplot(features, aes(x=misdem_count_person, y=violence_raw)) +
       geom_point(shape=1)  +
       ggtitle("Risk of Violence Raw vs Misdemeanors per Person (original)")
  
  #plot with no age  
ggplot(no_age_misdem, aes(x=misdem_count_person, y=raw_minus_age)) +
      geom_point(shape=1)  +
      ggtitle("Risk of Violence Raw vs Misdemeanors per Person (no age)")
